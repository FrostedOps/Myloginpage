...
import sqlite3, hashlib, secrets, datetime, socket
...
def add_time(current, duration):
    ...
    if duration == "week":
        return (base + datetime.timedelta(weeks=1)).isoformat()
    if duration == "month":
        return (base + datetime.timedelta(days=30)).isoformat()
    if duration == "year":
        return (base + datetime.timedelta(days=365)).isoformat()
...
def generate_key():
    ...
    c.execute("INSERT INTO keys VALUES (?,?,?,?,?,?)",
              (new_key, session["username"], 0, None, expiry, duration))
    conn.commit()
    conn.close()
